...
from a fixed `"eval"` stream. Comparing strategies on identical paths is standard Monte
Carlo variance reduction: the difference between two methods on a shared path set has far
lower variance than the difference of two independently estimated means. This costs nothing
and materially increases the power of every A-versus-B comparison in §7. Record it in the
protocol as a stated control.
